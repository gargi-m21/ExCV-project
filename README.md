...
A comparative medical computer vision and interpretability pipeline evaluating traditional grid-convolutional features (**DenseNet-121**) against global multi-head self-attention networks (**ViT-B16**) for 3-class chest radiograph classification (*COVID-19, Normal, and Viral Pneumonia*). This framework moves beyond qualitative aesthetics by computing structural localization (**Shannon Entropy**) and operational faithfulness (**Area Over Perturbation Curve - AOPC**) to quantitatively validate visual explanations.
...
│
├── .gitignore                   # Excludes large checkpoint binaries and metadata
...
Explanations are rigorously evaluated to prove they reflect the internal decision-making graph of the networks rather than diffuse background noise:

* **Saliency Map Shannon Entropy (DenseNet-121):** `5.0399 bits` (Confirms a highly localized explanation pattern concentrated within target pathological lung regions).
* **Global Dataset AOPC Score:** `0.2589` (Area Over Perturbation Curve computed dynamically over 50 test samples via iterative feature-level activation masking at the deepest convolutional outputs).
